<div class="profile-component-card" style="font-family: 'Inter', 'Segoe UI', system-ui, sans-serif;">
    <div class="px-5 pt-5 pb-2">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-bold text-slate-1000">{{ $type->type_name }}</h1>
                <p class="text-sm text-slate-500">{{ $type->abbreviation ?? '-' }}</p>
            </div>
            <div class="flex items-center gap-3">
                @if(\Illuminate\Support\Facades\Route::has('degree-types.edit'))
                    <a href="{{ route('degree-types.edit', ['degreetype_id' => $type->degreetype_id]) }}" class="text-gray-600 hover:text-gray-800" title="Edit">
                        <i class="fa fa-edit"></i>
                    </a>
                @endif
                <a href="{{ route('degree-types.index') }}" class="text-gray-600 hover:text-gray-800" title="Back">
    ...
    <div class="px-5 pb-5">
        <div class="space-y-6">
            ...
            <div class="bg-white rounded border border-slate-200 p-6">
                <h2 class="text-lg font-semibold text-slate-900 mb-4">Type Information</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <p class="text-sm text-slate-500">Type Name</p>
                        <p class="text-base font-medium text-slate-900">{{ $type->type_name }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-slate-500">Abbreviation</p>
                        <p class="text-base font-medium text-slate-900">{{ $type->abbreviation ?? '-' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded border border-slate-200 p-6">
                ...
                <div class="mb-6">
                    ...
                    <p class="text-sm text-slate-500">Users with programs using this type</p>
                </div>
