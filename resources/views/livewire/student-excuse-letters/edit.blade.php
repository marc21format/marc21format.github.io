<div class="profile-component-card">
    <div class="profile-card-header">
        <div>
            <p class="profile-card-title">Edit Excuse Letter</p>
            @php
        ...
        <form wire:submit.prevent="save" enctype="multipart/form-data">
            <div class="form-row">
                ...
                <div class="form-col w-2/3">
                    <label for="new_letter_file" class="form-label">Upload New Letter (PDF, DOC, DOCX, JPG, PNG)</label>
                    <input type="file" wire:model="new_letter_file" id="new_letter_file" class="form-control" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png">
                    @error('new_letter_file') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    @if($letter_link)
                        <p class="text-sm text-slate-600 mt-1">Current file: <a href="{{ route('download.excuse-letter', $letter->letter_id) }}" target="_blank" class="text-blue-600">{{ basename($letter_link) }}</a></p>
                    @endif
                </div>
            </div>
            <div class="form-row">
                <div class="form-col {{ auth()->user()->role_id != 4 ? 'w-1/3' : 'w-1/2' }}">
                    <label for="date_attendance" class="form-label">Date of Attendance</label>
                    <input type="date" wire:model="date_attendance" id="date_attendance" class="form-control">
