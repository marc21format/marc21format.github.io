@php
    $user = $user ?? ($this->user ?? (isset($userId) ? \App\Models\User::find($userId) : auth()->user()));
    $p = $user->userProfile ?? null;
    // Get a professional credential (if any) to include prefix/suffix titles
    ...
    $cancelRoute = route('profile.volunteer.show', ['user' => $userId ?? auth()->id()]);
@endphp

<div class="profile-component-card fill">
    <div class="profile-card-header">
        <div>
            <p class="profile-card-title">Add subject assignment</p>
            <p class="profile-card-subtitle">Of {{ $displayName }}</p>
        </div>
        <div class="profile-card-actions">
            <button type="button" class="gear-button" wire:click.prevent="store" title="Add">
