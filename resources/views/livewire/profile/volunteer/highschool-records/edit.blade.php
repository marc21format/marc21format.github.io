...
<div class="profile-component-card">
	<div class="profile-card-header">
		...
		<div class="profile-card-actions">
			...
			@php
				$targetId = isset($userId) ? $userId : auth()->id();
			...
			@endphp
			<a href="{{ $cancelRoute }}" class="gear-button text-slate-800" title="Cancel">
				<i class="fa fa-times" aria-hidden="true"></i>
			</a>
			<button type="button" class="gear-button text-red-600" onclick="if(confirm('Delete this record?')) { document.getElementById('delete-form').submit(); }" title="Delete">
	...
	<div class="border-t border-slate-200 pt-4">
		<form wire:submit.prevent="update">
			...
			<div class="form-row flex items-center gap-4">
				...
				<div class="form-col w-1/3 min-w-0">
					...
					@error('year_end') <x-input-error>{{ $message }}</x-input-error> @enderror
				</div>
			</div>
		...
		<form id="delete-form" method="POST" action="{{ route('users.highschool_records.destroy', [isset($userId) ? $userId : (isset($user) && isset($user->id) ? $user->id : auth()->id()), $recordId]) }}" style="display: none;">
			@csrf
			@method('DELETE')
		</form>
	</div>
