...
<div class="profile-component-card">
    <div class="profile-card-header">
        <div>
            <p class="profile-card-title">Add Educational Record</p>
            @php
    ...
    <div class="border-t border-slate-200 pt-4">
        <form wire:submit.prevent="save">
            ...
            <div class="form-group">
                ...
                <select wire:model="degreeprogram_id" wire:change="checkDegreeProgram($event.target.value)" id="degreeprogram_id" class="form-select">
                    <option value="" class="text-slate-400">Select a Degree Program...</option>
                    @foreach($degreePrograms as $p)
                        <option value="{{ $p->degreeprogram_id }}" data-level="{{ $p->degreeLevel->level_name ?? '' }}">{{ $p->full_degree_program_name }}</option>
                    @endforeach
                </select>
