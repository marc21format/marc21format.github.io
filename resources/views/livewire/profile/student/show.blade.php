@php
    ...
    $roleLabel = $roleLabel ?: ($roleId === 4 ? 'Student' : 'User');
@endphp
        ...
        <div class="grid gap-6 sm:grid-cols-[220px_1fr]">
            ...
            <div class="profile-main-card">
                ...
                <div class="hidden space-y-4" id="fceer">
                    @livewire('profile.fceer-profile', ['userId' => $user->id, 'role' => 'student'])
                </div>

                {{-- Personal Records Section --}}
                <div class="hidden space-y-4" id="personal">
                    @livewire('profile.personal-information', ['userId' => $user->id])
                    @livewire('profile.highschool-records.all', ['userId' => $user->id])
                    @livewire('profile.student.highschool-subject-records.all', ['userId' => $user->id])
                </div>
            </div>
        </div>
    </div>
