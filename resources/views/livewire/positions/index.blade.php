<div class="profile-component-card" style="font-family: 'Inter', 'Segoe UI', system-ui, sans-serif;">
    <div class="px-5 pt-5 pb-2">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                @if(\Illuminate\Support\Facades\Route::has('positions.create'))
                    <a href="{{ route('positions.create') }}" class="text-gray-600 hover:text-gray-800" title="Add position">
    ...
    <div class="px-5 pb-5">
                ...
                <tbody>
                    @foreach($this->positions as $p)
                        <tr class="border-b border-slate-100 hover:bg-slate-50">
                            <td class="p-3 text-slate-900">{{ $p->position_name ?? $p->position_name }}</td>
                            <td class="p-3 text-slate-900">{{ $p->committee_names ?? ($p->committee_names ?? '') }}</td>
                            <td class="p-3">
                                @if(\Illuminate\Support\Facades\Route::has('positions.edit'))
                                    <a href="{{ route('positions.edit', ['id' => $p->position_id ?? $p->position_id]) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-md text-slate-500 hover:bg-slate-50" title="Edit">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                @endif
                                <button wire:click="delete({{ $p->position_id }})" wire:confirm="Delete this position?" class="inline-flex items-center justify-center w-8 h-8 rounded-md text-red-500 hover:bg-red-50" title="Delete">
        ...
        @if(method_exists($this->positions, 'hasPages') && $this->positions->hasPages())
            <div class="mt-4">
